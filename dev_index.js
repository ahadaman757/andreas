      ...
      function (err, result) {
        if (err) {
          ...
          if (result.length > 0) {
            ...
            const expiry_date = 'SELECT DATE_ADD(DATE(NOW()), INTERVAL 8 DAY)'
            con.query(
              `INSERT INTO registered_users (f_name, l_name,email,password,c_name,expiry_date) VALUES (?, ?, ?, ?,?)`,
              [fname, lname, email, md5Pasword, company, expiry_date],
              function (err, result) {
                if (err) {
                  res.json({ Error: err });
                } else {
